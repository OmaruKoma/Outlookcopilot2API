...
from .models import TOOL_MESSAGE_TYPES
...
class M365Client:
    def __init__(self, token_manager: TokenManager, timeout_handshake=15,
                 timeout_recv=45, timeout_recv_final=60):
    ...
    async def _ensure_ws(self, conversation_id=None, hex_sid=None, x_session_id=None):
        token = self.token_manager.get()
        url, hex_sid, uuid_sid = build_url(token, hex_sid=hex_sid,
    ...
    async def chat(self, text, tone="Magic", conversation_id=None, hex_sid=None, x_session_id=None):
        ws, hex_sid, uuid_sid = await self._ensure_ws(conversation_id, hex_sid=hex_sid, x_session_id=x_session_id)
        payload = build_payload(hex_sid, uuid_sid, text, tone)
        return await self._send_recv(ws, payload)

    async def chat_stream(self, text, tone="Magic", conversation_id=None, hex_sid=None, x_session_id=None):
        full_text = ""
        async for chunk, is_final in self.chat_stream_gen(text, tone, conversation_id=conversation_id, hex_sid=hex_sid, x_session_id=x_session_id):
            if not is_final:
    ...
    async def chat_stream_gen(self, text, tone="Magic", conversation_id=None, hex_sid=None, x_session_id=None):
        ws, hex_sid, uuid_sid = await self._ensure_ws(conversation_id, hex_sid=hex_sid, x_session_id=x_session_id)
        payload = build_payload(hex_sid, uuid_sid, text, tone)
    ...
    async def chat_conversation(self, messages, tone="Magic", conversation_id=None,
                                hex_sid=None, x_session_id=None):
        result_text = ""
        async for chunk, is_final in self.chat_conversation_stream_gen(
            messages, tone, conversation_id=conversation_id, hex_sid=hex_sid, x_session_id=x_session_id):
            if not is_final:
                result_text += chunk
        return clean_text(result_text), self._last_tool_calls, self._last_finish_reason

    async def chat_conversation_stream_gen(self, messages, tone="Magic",
                                           conversation_id=None, hex_sid=None, x_session_id=None):
        ws, hex_sid, uuid_sid = await self._ensure_ws(conversation_id, hex_sid=hex_sid, x_session_id=x_session_id)
        payload = build_conversation_payload(hex_sid, uuid_sid, messages, tone)
        await ws.send(payload + "\x1e")
        tool_calls = []
        self._last_tool_calls = []
        self._last_full_text = ""
